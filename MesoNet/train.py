...
def train_and_plot(model, dataGenerator, x_train, y_train):
    initial_epoch = 1
    slow_epoch = 4
    normal_epoch = 16
    ...
    loss, accuracy = train_model(model, dataGenerator, x_train, y_train, initial_epoch)

    model.unfreeze()
    model.set_lr(1e-4)
    print("Train all layers with slow lr to adjust weights...")
    loss2, accuracy2 = train_model(model, dataGenerator, x_train, y_train, slow_epoch)
    loss.extend(loss2)
    accuracy.extend(accuracy2)

    model.set_lr(1e-3)
    print("Train all layers with normal lr...")
    loss3, accuracy3 = train_model(model, dataGenerator, x_train, y_train, normal_epoch)
    loss.extend(loss3)
...
def main():
    ...
    train_and_plot(meso4, dataGenerator, x_train, y_train)

    # MesoInception4
    mesoInception4 = MesoInception4()
    mesoInception4.load('weights/MesoInception4_DF')
    print("Now training: mesoInception4")
    train_and_plot(mesoInception4, dataGenerator, x_train, y_train)
